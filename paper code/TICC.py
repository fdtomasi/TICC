...
prefix_string = "data_lambda=" + str(lambda_parameter) + "beta = " + str(
    beta) + "clusters=" + str(number_of_clusters) + "/"
...
hexadecimal_color_list = [
    "cc0000", "0000ff", "003300", "33ff00", "00ffcc", "ffff00", "ff9900",
    "ff00ff", "cccc66", "666666", "ffccff", "660000", "00ff00", "ffffff",
    "3399ff", "006666", "330000", "ff0000", "cc99ff", "b0800f", "3bd9eb",
    "ef3e1b"
]
...
def computeF1Score_delete(num_cluster,
                          matching_algo,
                          actual_clusters,
                          threshold_algo,
                          save_matrix=False):
    ...
    for cluster in xrange(num_cluster):
        ...
        if save_matrix:
            np.savetxt("estimated_matrix_cluster=" + str(cluster) + ".csv",
                       estimated_matrix,
                       delimiter=",",
                       fmt="%1.4f")
        TP = 0
...
def compute_confusion_matrix(num_clusters, clustered_points_algo,
                             sorted_indices_algo):
    """
...
def computeNetworkAccuracy(matching, train_cluster_inverse, num_clusters):
    ...
    f1 = 0
    for cluster in xrange(num_clusters):
        true_cluster_cov = np.loadtxt("Inverse Covariance cluster =" +
                                      str(cluster) + ".csv",
                                      delimiter=",")
        matched_cluster = matching[cluster]
        ...
        for i in xrange(nrow):
            for j in xrange(ncol):
                if np.abs(matched_cluster_cov[i, j]) > threshold:
                    out_matched[i, j] = 1
        np.savetxt("Network_true_cluster=" + str(cluster) + ".csv",
                   true_cluster_cov,
                   delimiter=",")
        np.savetxt("Network_matched_cluster=" + str(matched_cluster) + ".csv",
                   matched_cluster_cov,
                   delimiter=",")
...
for iters in xrange(maxIters):
    ...
    if iters == 0:
        ...
        training_idx = np.random.choice(m - num_blocks + 1,
                                        size=int(m * training_percent),
                                        replace=False)
        ...
        true_confusion_matrix_g = compute_confusion_matrix(
            num_clusters, gmm_clustered_pts, sorted_training_idx)
        true_confusion_matrix_k = compute_confusion_matrix(
            num_clusters, clustered_points_kmeans, sorted_training_idx)
    ...
    for cluster in xrange(num_clusters):
        if len_train_clusters[cluster] != 0:
            ...
            gvx.AddEdge(0,
                        1,
                        Objective=lamb * dummy + num_stacked * dummy +
                        size_blocks * dummy)

            ##solve using customized ADMM solver
            gvx.Solve(Verbose=False,
                      MaxIters=1000,
                      Rho=1,
                      EpsAbs=1e-6,
                      EpsRel=1e-6)
            ...
            log_det_values[num_clusters, cluster] = np.log(
                np.linalg.det(cov_out))
            computed_covariance[num_clusters, cluster] = cov_out
            cluster_mean_info[num_clusters, cluster] = np.mean(
                D_train,
                axis=0)[(num_stacked - 1) * n:num_stacked * n].reshape([1, n])
            cluster_mean_stacked_info[num_clusters, cluster] = np.mean(D_train,
                                                                       axis=0)
    ...
    for cluster in xrange(num_clusters):
        print "length of the cluster ", cluster, "------>", len_train_clusters[
            cluster]
    ##Computing the norms
    if iters != 0:
        for cluster in xrange(num_clusters):
            cluster_norms[cluster] = (np.linalg.norm(
                old_computed_covariance[num_clusters, cluster]), cluster)
        ...
        if len_train_clusters[cluster] == 0:
            ...
            while not break_flag:
                ...
                if clustered_points[point_num] == cluster_selected:
                    ...
                    computed_covariance[num_clusters,
                                        cluster] = old_computed_covariance[
                                            num_clusters, cluster_selected]
                    cluster_mean_stacked_info[
                        num_clusters, cluster] = complete_D_train[point_num, :]
                    cluster_mean_info[num_clusters,
                                      cluster] = complete_D_train[point, :][
                                          (num_stacked - 1) * n:num_stacked *
                                          n]
                    break_flag = True
            counter += 1
    ...
    for point in xrange(len(clustered_points)):
        ...
        if point + num_stacked - 1 < complete_D_train.shape[0]:
            for cluster in xrange(num_clusters):
                ...
                cluster_mean_stacked = cluster_mean_stacked_info[num_clusters,
                                                                 cluster]

                x = complete_D_train[point, :] - cluster_mean_stacked[0:(
                    num_blocks - 1) * n]
                cov_matrix = computed_covariance[num_clusters, cluster][0:(
                    num_blocks - 1) * n, 0:(num_blocks - 1) * n]
                inv_cov_matrix = np.linalg.inv(cov_matrix)
                log_det_cov = np.log(
                    np.linalg.det(cov_matrix))  # log(det(sigma2|1))
                lle = np.dot(
                    x.reshape([1, (num_blocks - 1) * n]),
                    np.dot(inv_cov_matrix, x.reshape([n * (num_blocks - 1), 1
                                                      ]))) + log_det_cov
                LLE_all_points_clusters[point, cluster] = lle

    ##Update cluster points - using dynamic programming smoothening
    clustered_points = updateClusters(LLE_all_points_clusters,
                                      switch_penalty=switch_penalty)
    print "\ncompleted smoothening algorithm"
    print "\n\nprinting the length of points in each cluster"

    for cluster in xrange(num_clusters):
        print "length of cluster #", cluster, "-------->", sum(
            [x == cluster for x in clustered_points])
    true_confusion_matrix = np.zeros([num_clusters, num_clusters])

    ##Save a figure of segmentation
    plt.figure()
    plt.plot(sorted_training_idx[0:len(clustered_points)],
             clustered_points,
    ...
    plt.figure()
    plt.plot(sorted_training_idx[0:len(gmm_clustered_pts)],
             gmm_clustered_pts,
             color="r")  #,marker=".",s=100)
    plt.ylim((-0.5, num_clusters + 0.5))
    ...
    true_confusion_matrix = compute_confusion_matrix(num_clusters,
                                                     clustered_points,
                                                     sorted_training_idx)
    ...
    for point in xrange(len(clustered_points_test)):
        ...
        for cluster in xrange(num_clusters):
            ...
            vals[cluster] = lle + switch_penalty * (
                cluster != left_clust) + switch_penalty * (cluster !=
                                                           right_clust)
        out = np.argmin(vals)
        clustered_test[point] = out

    plt.figure()
    plt.plot(sorted_test_idx[0:len(clustered_test)], clustered_test,
             color="r")  #,marker = ".", s= 100)
    plt.ylim((-0.5, num_clusters + 0.5))
    ...
    for point in xrange(len(clustered_points_test)):
        ...
        for cluster in xrange(num_clusters):
            ...
            x = data_tt - cluster_mean_stacked[0:(num_blocks - 1) * n]
            cov_matrix = gmm_covariances[cluster][0:(num_blocks - 1) *
                                                  n, 0:(num_blocks - 1) * n]
            inv_cov_matrix = np.linalg.inv(cov_matrix)
            ...
            vals[
                cluster] = lle  #+ switch_penalty*(cluster !=left_clust ) + switch_penalty*(cluster != right_clust )
        out = np.argmin(vals)
        clustered_test_gmm[point] = out

    plt.figure()
    plt.plot(sorted_test_idx[0:len(clustered_test_gmm)],
             clustered_test_gmm,
             color="r")  #,marker = ".", s= 100)
    plt.ylim((-0.5, num_clusters + 0.5))
    ...
    plt.figure()
    plt.plot(sorted_test_idx[0:len(clustered_points_test_kmeans)],
             clustered_points_test_kmeans,
             color="r")  #,marker = ".", s= 100)
    plt.ylim((-0.5, num_clusters + 0.5))
    ...
    ##Segment length
    seg_len = 50
    true_confusion_matrix_EM = compute_confusion_matrix(
        num_clusters, clustered_test, sorted_test_idx)
    true_confusion_matrix_GMM = compute_confusion_matrix(
        num_clusters, gmm_clustered_pts_test, sorted_test_idx)
    true_confusion_matrix_kmeans = compute_confusion_matrix(
        num_clusters, clustered_points_test_kmeans, sorted_test_idx)
    ...
    plt.close("all")
    binary_EM = (true_confusion_matrix_EM[0, 0] +
                 true_confusion_matrix_EM[1, 1]) / len(clustered_points_test)
    binary_EM = np.max([binary_EM, 1 - binary_EM])
    # print "EM is -------->",binary_EM
    binary_GMM = (true_confusion_matrix_GMM[0, 0] +
                  true_confusion_matrix_GMM[1, 1]) / len(clustered_points_test)
    binary_GMM = np.max([binary_GMM, 1 - binary_GMM])

    binary_Kmeans = (
        true_confusion_matrix_kmeans[0, 0] +
        true_confusion_matrix_kmeans[1, 1]) / len(clustered_points_test)
    ...
    for cluster in xrange(num_clusters):
        ...
        train_inverse_covariance_gmm[cluster] = np.linalg.inv(
            gmm_covariances[cluster])
...
##Assume its a 2x2 matrix?
    ...
    for cluster in xrange(num_clusters):
        ...
        actual_clusters[cluster] = np.loadtxt("Inverse Covariance cluster =" +
                                              str(cluster) + ".csv",
                                              delimiter=",")
...
##compute the appropriate matching
    ...
    for cluster in xrange(num_clusters):
        ...
        correct_KMeans += true_confusion_matrix_kmeans[cluster,
                                                       matched_cluster_Kmeans]
...
train_confusion_matrix_EM = compute_confusion_matrix(num_clusters,
                                                     clustered_points,
                                                     sorted_training_idx)
train_confusion_matrix_GMM = compute_confusion_matrix(num_clusters,
                                                      gmm_clustered_pts,
                                                      sorted_training_idx)
train_confusion_matrix_kmeans = compute_confusion_matrix(
    num_clusters, clustered_points_kmeans, sorted_training_idx)
test_confusion_matrix_EM = compute_confusion_matrix(num_clusters,
                                                    clustered_test,
                                                    sorted_test_idx)
...
# print " \n"
f1_EM_tr = computeF1_macro(train_confusion_matrix_EM, matching_EM,
                           num_clusters)
f1_GMM_tr = computeF1_macro(train_confusion_matrix_GMM, matching_GMM,
                            num_clusters)
f1_kmeans_tr = computeF1_macro(train_confusion_matrix_kmeans, matching_Kmeans,
                               num_clusters)
f1_EM_test = computeF1_macro(test_confusion_matrix_EM, matching_EM,
                             num_clusters)
...
for cluster in xrange(num_clusters):
    ...
    correct_KMeans += train_confusion_matrix_kmeans[cluster,
                                                    matched_cluster_Kmeans]
