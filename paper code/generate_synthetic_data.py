...
def generate_inverse(rand_seed):
    ...
    def genInvCov(size, low=0.3, upper=0.6, portion=0.2, symmetric=True):
        ...
        for EI in G.Edges():
            value = (np.random.randint(2) -
                     0.5) * 2 * (low + (upper - low) * np.random.rand(1)[0])
            # print value
    ...
    for block in xrange(num_blocks):
        if block == 0:
            block_matrices[block] = genInvCov(size=size_blocks,
                                              portion=sparsity_inv_matrix,
                                              symmetric=(block == 0))
        else:
            block_matrices[block] = genRandInv(size=size_blocks,
                                               portion=sparsity_inv_matrix)
...
for cluster in xrange(num_clusters):
    ...
    if save_inverse_covarainces:
        np.savetxt("Inverse Covariance cluster =" + str(cluster) + ".csv",
                   cluster_inverses[cluster],
                   delimiter=",",
                   fmt='%1.6f')
        np.savetxt("Covariance cluster =" + str(cluster) + ".csv",
    ...
    for num in xrange(old_break_pt, break_pt):
        ...
        if num == 0:
            cov_matrix = cluster_covariances[cluster][
                0:size_blocks, 0:size_blocks]  ##the actual covariance matrix
            ...
            new_row = np.random.multivariate_normal(
                new_mean.reshape(size_blocks), cov_matrix)
            Data[num, :] = new_row
            ...
            ##The first section
            cov_matrix = cluster_covariances[cluster][
                0:(num + 1) * size_blocks, 0:(num + 1) *
                size_blocks]  ##the actual covariance matrix
            n = size_blocks
            Sig22 = cov_matrix[(num) * n:(num + 1) * n, (num) * n:(num + 1) *
                               n]
            Sig11 = cov_matrix[0:(num) * n, 0:(num) * n]
            Sig21 = cov_matrix[(num) * n:(num + 1) * n, 0:(num) * n]
            Sig12 = np.transpose(Sig21)
            cov_mat_tom = Sig22 - np.dot(np.dot(Sig21, np.linalg.inv(Sig11)),
                                         Sig12)  #sigma2|1
            ...
            for idx in xrange(num):
                a[idx * size_blocks:(idx + 1) *
                  size_blocks, 0] = Data[idx, :].reshape([size_blocks])
            new_mean = cluster_mean + np.dot(
                np.dot(Sig21, np.linalg.inv(Sig11)),
                (a - cluster_mean_stacked[0:(num) * size_blocks, :]))
            new_row = np.random.multivariate_normal(
                new_mean.reshape(size_blocks), cov_mat_tom)
            Data[num, :] = new_row
            ...
            if num == 2:
                ...
                print "a is:", a
                print "second term is:", np.dot(
                    np.dot(Sig21, np.linalg.inv(Sig11)),
                    (a - cluster_mean_stacked[0:(num) * size_blocks, :]))
                print "first term is:", cluster_mean

        else:
            cov_matrix = cluster_covariances[
                cluster]  # actual covariance matrix
            n = size_blocks
            ...
            for idx in xrange(num_blocks - 1):
                a[idx * size_blocks:(idx + 1) *
                  size_blocks, 0] = Data[num - num_blocks + 1 +
                                         idx, :].reshape([size_blocks])
            ...
            new_mean = cluster_mean + np.dot(
                np.dot(Sig21, np.linalg.inv(Sig11)),
                (a -
                 cluster_mean_stacked[0:(num_blocks - 1) * size_blocks, :]))
