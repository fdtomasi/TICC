...
class TICC(BaseEstimator):
    def __init__(
            self, window_size=10, number_of_clusters=5, lambda_parameter=11e-2,
            beta=400, maxIters=1000, threshold=2e-5, write_out_file=False,
            prefix_string="", num_proc=1, compute_BIC=False,
            cluster_reassignment=20, verbose=False):
        """
        Parameters:
    ...
    def fit(self, X):
        ...
        for iters in range(self.maxIters):
            ...
            self.optimize_clusters(
                computed_covariance, len_train_clusters, log_det_values,
                opt_res, train_cluster_inverse)
            ...
            clustered_points = self.predict_clusters().astype(int)
            ...
            if iters != 0:
                cluster_norms = [
                    (
                        ...
                        ), i) for i in range(self.number_of_clusters)
                ]
                ...
                for cluster_num in range(self.number_of_clusters):
                    if len_new_train_clusters[cluster_num] == 0:
                        ...
                        if self.verbose > 1:
                            print(
                                "cluster that is zero is:", cluster_num,
        ...
        self.compute_f_score(
            matching_EM, matching_GMM, matching_Kmeans,
            train_confusion_matrix_EM, train_confusion_matrix_GMM,
            train_confusion_matrix_kmeans)

        if self.compute_BIC:
            bic = computeBIC(
                self.number_of_clusters, time_series_rows_size,
    ...
    def write_plot(self, clustered_points, str_NULL, training_indices):
        ...
        plt.figure()
        plt.plot(
            training_indices[0:len(clustered_points)], clustered_points,
            color="r")  # ,marker = ".",s =100)
        plt.ylim((-0.5, self.number_of_clusters + 0.5))
        if self.write_out_file:
            plt.savefig(
                str_NULL + "TRAINING_EM_lam_sparse=" +
                str(self.lambda_parameter) + "switch_penalty = " +
    ...
    def smoothen_clusters(
            self, cluster_mean_info, computed_covariance,
        ...
        for point in range(clustered_points_len):
            if point + self.window_size - 1 < complete_D_train.shape[0]:
                for cluster in range(self.number_of_clusters):
                    cluster_mean = cluster_mean_info[self.number_of_clusters,
                                                     cluster]
    ...
    def optimize_clusters(
            self, computed_covariance, len_train_clusters, log_det_values,
            optRes, train_cluster_inverse):
        for cluster in range(self.number_of_clusters):
        ...
        for cluster in range(self.number_of_clusters):
            if self.verbose > 1:
                print(
                    "length of the cluster ", cluster, "------>",
                    len_train_clusters[cluster])

    def train_clusters(
            self, cluster_mean_info, cluster_mean_stacked_info,
            complete_D_train, empirical_covariances, len_train_clusters, n,
        ...
        for cluster in range(self.number_of_clusters):
            ...
            if cluster_length != 0:
                ...
                cluster_mean_info[self.number_of_clusters, cluster] = np.mean(
                    D_train, axis=0)[(self.window_size - 1) *
                                     n:self.window_size * n].reshape([1, n])
                cluster_mean_stacked_info[self.number_of_clusters,
                                          cluster] = np.mean(D_train, axis=0)
    ...
    def prepare_out_directory(self):
        str_NULL = self.prefix_string + "lam_sparse=" + str(
            self.lambda_parameter) + "maxClusters=" + str(
    ...
    def load_data(self, input_file):
        ...
        (
            m, n
        ) = Data.shape  # m: num of observations, n: size of observation vector
        if self.verbose:
